...
def _promise_cards(trusted: list[dict], n: int = 48) -> list[dict]:
    ...
    for r in picked:
        ...
        # The verdict must be a function of the number we PUBLISH: deriving
        # it from the unrounded error once produced a card whose rounded
        # err_min said "kept" while its verdict said "late", which the
        # sanity contract rightly refused to publish (and the observatory
        # chain went down for it). One value, one truth.
        err = round(wait - h, 1)
